...
class ResNet(nn.Module):
    ...
    def __init__(self, layers, block, num_classes=1000, zero_init_residual=True, initial_conv='1x7',
...
class ArchUncertResNet(ResNet):
    ...
    def __init__(self, layers, start_depth, end_depth, block=Bottleneck, 
                 num_classes=1000, zero_init_residual=True, initial_conv='1x7',
                 concat_pool=False, input_chanels=3, p_drop=0):
        super(ArchUncertResNet, self).__init__(layers, block, num_classes=num_classes,
    ...
    def _forward_impl(self, x):
        # See note [TorchScript super()]
        x = self.fwd_input_block(x) # Size [256, 256, 28, 28]

        act_vec = x.new_zeros(self.n_layers, x.shape[0], self.num_classes) # Size [12, 256, 10]
        for layer_idx in range(self.start_depth, self.end_depth):
            layer = self.layer_list[layer_idx]
            x = layer(x) # Size [256, 256, 28, 28]
            y = self.adapt_layers[self.channel_list[layer_idx]](x) # Size [256, 10]
            y = self.fwd_output_block(y)
            act_vec[layer_idx-self.start_depth, :, :] = y # Size [12, 256, 10]
        return act_vec
